...
if __name__ == '__main__':
    ...
    if (total != 2):
        ...
        scoop_dtl = sqlContext.sql("CREATE EXTERNAL TABLE IF NOT EXISTS scoop_dtl (precimaId string,divisionNumber string,customerNumber string,productNumber string,prod_desc string,prod_brnd string,pack_size string,qty_uom string,sales_opp string,prc_guidance string,pimMerchandisingCategoryDescription string,pimMerchandisingCategoryId string, productRank string) ROW FORMAT DELIMITED FIELDS TERMINATED BY  '\t' LOCATION 's3://krutisatish/main_file/' TBLPROPERTIES ('serialization.null.format'='', 'skip.header.line.count'='2')")
        scoop_hdr = sqlContext.sql("CREATE EXTERNAL TABLE IF NOT EXISTS scoop_hdr (precimaId string,description string,c string,startDate string,endDate string,divisionNumber string,customerNumber string) ROW FORMAT DELIMITED FIELDS TERMINATED BY  '\t' LOCATION 's3://krutisatish/hdr_file/' TBLPROPERTIES ('serialization.null.format'='', 'skip.header.line.count'='2')")
